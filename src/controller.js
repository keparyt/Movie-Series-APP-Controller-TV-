...
function action(index) {
  ...
  if (isPlayer()) { sendPlayerKey(key); return; }
...
function poll() {
  ...
  if (pad && (!activePad || activePad.index !== pad.index)) {
    activePad = { index:pad.index, id:pad.id }; previous = []; lastAction.clear();
  ...
  const x = Math.abs(pad.axes?.[0] || 0) > .42 ? Math.sign(pad.axes[0]) : 0;
  const y = Math.abs(pad.axes?.[1] || 0) > .42 ? Math.sign(pad.axes[1]) : 0;
...
window.addEventListener('gamepadconnected', event => { activePad={index:event.gamepad.index,id:event.gamepad.id}; previous=[]; setStatus(`🎮 Controller detected · ${event.gamepad.id || 'Gamepad'}`); emitControllerActivity({connected:true}); });
window.addEventListener('gamepaddisconnected', event => { if (!activePad || activePad.index===event.gamepad.index) { activePad=null; setStatus('🎮 Controller disconnected',false); window.dispatchEvent(new CustomEvent('controllerdisconnected')); } });
